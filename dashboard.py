...
def _inject_custom_style() -> None:
    ...
    st.markdown(
        ...
        /* Метрики: не переопределяем стили, чтобы компоненты корректно отображались
...
def display_dashboard(sheet_id: Optional[str] = None) -> None:
    ...
    # Сделки считаем только для строк, где номер ДС поставщика отсутствует (это сделки контрагента).
    # Даже если у контрагента номер ДС отсутствует (NaN), строка всё равно учитывается.
